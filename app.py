...
FIXED VERSION - Resolved file upload and routing issues
"""
...
# Configuration - FIXED: Increased file size limit
app.config['MAX_CONTENT_LENGTH'] = 100 * 1024 * 1024  # 100MB max file size (increased from 50MB)
app.config['UPLOAD_FOLDER'] = 'uploads'
app.config['ALLOWED_EXTENSIONS'] = {'pdf', 'docx', 'txt', 'jpg', 'jpeg', 'png', 'mp3', 'mp4', 'wav', 'doc', 'xlsx', 'xls', 'csv'}
...
# FIXED: Serve index.html for all routes (handles refresh issue)
@app.route('/')
@app.route('/<path:path>')
def serve_spa(path=''):
    """Serve the main HTML page - handles all routes for SPA"""
    try:
        # If requesting a specific file that exists, serve it
        if path and os.path.exists(path):
            return send_file(path)
        
        # Otherwise, always serve index.html (for SPA routing)
...
def generate_report():
    ...
    try:
        ...
        if 'rubric' not in exam_config:
            exam_config['rubric'] = {'total_points': 100}
...
def upload_files():
    ...
    try:
        ...
        for idx, file in enumerate(files, 1):
            try:
                if not file:
                    logger.warning(f"⚠️  File {idx} is None, skipping")
                    continue
                    
                if not file.filename:
                    logger.warning(f"⚠️  File {idx} has no filename, skipping")
                    continue
        ...
        return jsonify({
            ...
            'details': 'Unexpected error during file upload'
        }), 500
...
def grade_submission():
    ...
    try:
        ...
        model_answer_mode = data.get('modelAnswerMode', 'type')
